...
class IBLT:
    ...
    def __init__(self, m, k, key_size, value_size, hash_key_sum_size=10, hash=None):
        ...
        self.T = [
            [0, [0 for j in range(key_size)], [0 for j in range(value_size)], [0 for j in range(hash_key_sum_size)]] for
            i in range(m)]
        # print("len");
        # print(len(self.T));
        # print(self.T);
    ...
    def __insert(self, T, key, value):
        ...
        for index in indices:
            ...
            T[index][3] = self.__sum_int_arrays(T[index][3], self.__value_to_int_array(IBLT.get_key_hash(key),
    ...
    def __delete(self, T, key, value):
        ...
        for index in indices:
            ...
            T[index][3] = self.__diff_int_arrays(T[index][3], self.__value_to_int_array(IBLT.get_key_hash(key),
                                                                                        self.hash_key_sum_size))

    def subtract(self, arr2):
        """
        Subtract 2 arrays and list the elements .
        Elements printed are A-B and B-A.
        """
        self_copy = deepcopy(self)
        t = self_copy.__subtract(arr2.T)
        res = IBLT(self.m, self.k, self.key_size, self.value_size, self.hash_key_sum_size, self.hash)
        res.T = t
        return res

    def __subtract(self, arr2):
        for i in range(0, len(self.T)):
            self.T[i][0] = self.T[i][0] - arr2[i][0]
            self.T[i][1] = self.__diff_int_arrays(self.T[i][1], arr2[i][1])
            self.T[i][2] = self.__diff_int_arrays(self.T[i][2], arr2[i][2])
            self.T[i][3] = self.__diff_int_arrays(self.T[i][3], arr2[i][3])
        return self.T
        ...
        for index in indices:
            if self.T[index][0] == 0 and \
                ...
                return (IBLT.RESULT_GET_MATCH, self.__int_array_to_value(self.T[index][2]))
                    ...
                    self.T[index][3] == self.__negate_int_array(
                self.__value_to_int_array(IBLT.get_key_hash(key), self.hash_key_sum_size)):
                return (
                    IBLT.RESULT_GET_DELETED_MATCH, self.__int_array_to_value(self.__negate_int_array(self.T[index][2])))
        ...
        while True:
            for i in range(len(T)):
                ...
                if entry[0] == 1 or entry[0] == -1:
                    if entry[0] == 1 and \
                            entry[3] == self.__value_to_int_array(
                        IBLT.get_key_hash(self.__int_array_to_value(entry[1])), self.hash_key_sum_size):
                        ...
                        break
                            ...
                            self.__negate_int_array(entry[3]) == self.__value_to_int_array(
                        IBLT.get_key_hash(self.__int_array_to_value(self.__negate_int_array(entry[1]))),
                        self.hash_key_sum_size):
    ...
    def serialize(self):
        ...
        for cell in self.T:
            ...
            # keySum
            data += b"".join(list(map(lambda n: struct.pack(">B", n), cell[1])))
            # valueSum
            data += b"".join(list(map(lambda n: struct.pack(">B", n), cell[2])))
            # hashKeySum
            data += b"".join(list(map(lambda n: struct.pack(">B", n), cell[3])))
    ...
    def unserialize(data: bytes):
        ...
        for i in range(m):
            ...
            offset += 4
            t.T[i][1] = list(
                map(lambda c: struct.unpack(">B", c.to_bytes(1, "big"))[0], data[offset:offset + key_size]))
            offset += key_size
            t.T[i][2] = list(
                map(lambda c: struct.unpack(">B", c.to_bytes(1, "big"))[0], data[offset:offset + value_size]))
            offset += value_size
            t.T[i][3] = list(
                map(lambda c: struct.unpack(">B", c.to_bytes(1, "big"))[0], data[offset:offset + hash_key_sum_size]))
    ...
    def __dump(t):
        ...
        print("Non-empty count: %d" % len(list(filter(lambda e: e[0] > 0, t))))
        print("Below-zero sum: %d" % sum(map(lambda e: e[0], filter(lambda e: e[0] < 0, t))))
        print("Below-zero count: %d" % len(list(filter(lambda e: e[0] < 0, t))))
        for i in range(len(t)):
            e = t[i]
            print("%d: %s" % (i, e))
        print("DUMP END::::::::::::::::::::::::")
    ...
    def __hash(self, i, value):
        ...
        return int(hashlib.sha512((str(i) + value).encode()).hexdigest()[:self.hash_hex_length], 16) % self.m
...
#
#
# result, elements, deleted = a.list_entries()
#
# ser = a.serialize()
# des = IBLT.unserialize(ser)
#
# # result, elements, deleted = des.list_entries()
# #
# # print(elements)
#
# a = des
...
# b.insert('3'*40, '4'*64)
#
# result, elements, deleted = b.list_entries()
#
# print(result)
# print(elements)
# print(deleted)
...
# c = a.subtract(b)
#
# result, elements, deleted = c.list_entries()
#
# print(result)
# print(elements)
# print(deleted)
